...
serve(async (req) => {
  ...
  try {
    ...
    // Support both authenticated calls and service-to-service calls
    const authHeader = req.headers.get("Authorization");
    const supabaseUrl = Deno.env.get("SUPABASE_URL")!;
    const serviceRoleKey = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;

    // Try user auth first, fall back to service client
    ...
    if (identity?.clone_status !== "ready") {
      return new Response(
        JSON.stringify({ error: "Avatar is not ready yet. Current status: " + (identity?.clone_status || "unknown") }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
